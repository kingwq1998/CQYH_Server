...
namespace 游戏服务器.地图类
{
	public sealed class 怪物实例 : 地图对象
	{
		...
		public override void 自身死亡处理(地图对象 对象, bool 技能击杀, bool 脚本击杀 = false)
		{
			...
			// 尸王殿入口: 不依赖触发lua, 对 154 地图所有怪物均计数 (普通小怪无触发lua导致Lua版失效)
			if (Settings.尸王殿开启击杀数 > 0 && this.当前地图 != null && this.当前地图.地图编号 == 154 && this.模板编号 != 8479)
			{
				int 计数槽 = 3000;
				if (!this.当前地图.数字变量.ContainsKey(计数槽))
					this.当前地图.数字变量[计数槽] = 0;
				this.当前地图.数字变量[计数槽]++;
				if (this.当前地图.数字变量[计数槽] >= Settings.尸王殿开启击杀数)
				{
					this.当前地图.数字变量[计数槽] = 0;
					游戏怪物 门模板 = 游戏怪物.获取游戏怪物("尸王殿入口");
					if (门模板 != null)
					{
						怪物实例 门 = new 怪物实例(门模板, this.当前地图, 60000, this.当前坐标, 禁止复活: true, 立即刷新: true);
						if (Settings.尸王殿门存续秒数 > 0)
							门.存活时间 = 主程.当前时间.AddSeconds(Settings.尸王殿门存续秒数);
						网络服务网关.发送公告("比奇矿区深处魔气翻涌，尸王殿传送门已开启！限时开放，勇者速往！");
					}
				}
			}
			base.自身死亡处理(对象, 技能击杀);
			...
			{
				if (this.怪物级别 == 怪物级别分类.精英干将 || this.怪物级别 == 怪物级别分类.头目首领 || this.对象模板.刷新通知 || 首杀怪物名单.数据表.Contains(this.模板编号))
				{
					主程.添加系统日志($"{this.完整名字} {this.地图编号} 被 {归属玩家.角色数据.角色名字.V} 击杀,下次复活 {this.复活时间.ToString()}");
					// 首杀(从零原创): BOSS 级(精英/首领/刷新通知) 或 首杀怪物名单.csv 显式列入的怪 纳入全服首杀统计
					首杀首爆管理器.尝试怪物首杀(this.模板编号, this.对象模板.怪物名字, 归属玩家);
				}
				...
				if (num3 < 1f)
				{
					foreach (怪物掉落 item5 in 计算类.RandomSort(this.怪物掉落))
					{
						...
						if (value2.贵重物品 && !value2.爆不通知)
						{
							...
							// 首爆(从零原创): 仅贵重物品纳入全服首爆统计
							首杀首爆管理器.尝试装备首爆(value2.物品编号, value2.物品名字, 归属玩家);
						}
					}
				}
				if (!脚本击杀)
