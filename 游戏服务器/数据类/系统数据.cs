...
namespace 游戏服务器.数据类
{
	public class 系统数据 : 游戏数据
	{
		...
		private static void 更新榜单(列表监视器<角色数据> 当前榜单, byte 当前类型, object 角色, IComparer<角色数据> 比较方法)
		{
			...
			// 防御: 持久化的旧排名(当前排名)可能因榜单尺寸变化而失效 —— 内存假人(假人网关, 不落盘)
			// 重启后从榜单脱落致榜单变短, 但持久化角色身上的旧排名未同步, 按旧索引 RemoveAt/遍历会错位越界、
			// 触发空引用崩溃(机器人初始化进榜场景)。以角色在榜单中的真实位置为准: 旧排名对不上就重新定位。
			if (num < 0 || num >= 当前榜单.Count || 当前榜单[num] != (角色数据)角色)
			{
				num = -1;
				for (int 定位 = 0; 定位 < 当前榜单.Count; 定位++)
				{
					if (当前榜单[定位] == (角色数据)角色)
					{
						num = 定位;
