...
namespace 游戏服务器.数据类
{
    ...
    public sealed class 角色数据 : 游戏数据
    {
        ...
        // 运行时标记: 该角色是否为假人(内存隔离、不落盘, 见 假人网关.cs)。
        // 普通字段, 非 数据监视器, 不进持久化映射, 仅运行时使用 —— 用于序列化时把假人挡在存档外。
        public bool 是否假人;
        ...
        {
            if (this.网络连接 != null && this.网络连接 != 网络)
            {
                this.网络连接.绑定角色?.玩家角色下线();
                this.网络连接.尝试断开连接(new Exception("角色被新连接接管, 旧连接被踢"));
            }
            this.网络连接 = 网络;
            网络服务网关.已上线连接数++;
            this.物理地址.V = 网络?.物理地址;
