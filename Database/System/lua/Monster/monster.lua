...
function monster_die(怪物, 击杀者, 技能击杀)
    ...
    -- 尸王殿入口: 比奇矿区三层(154)累计击杀达阈值, 在最后击杀处开启一道限时传送门(8479尸王殿入口)并全服公告。
    -- 阈值 尸王殿开启击杀数 / 存续 尸王殿门存续秒数 由 游戏脚本.cs 从 Settings 注入为 Lua 全局; 0 则关闭该玩法。
    if 怪物.当前地图.地图编号 == 154 and 尸王殿开启击杀数 ~= nil and 尸王殿开启击杀数 > 0 then
        local 地图 = 怪物.当前地图
        local 计数槽 = 3000  -- 该地图 数字变量 里的计数槽位key(仅作key, 与阈值解耦; 原stub把它既当key又当阈值是bug)
        if not 地图.数字变量:ContainsKey(计数槽) then
            地图.数字变量[计数槽] = 0
        end
        地图.数字变量[计数槽] = 地图.数字变量[计数槽] + 1
        if 地图.数字变量[计数槽] >= 尸王殿开启击杀数 then
            地图.数字变量[计数槽] = 0
            local 门 = 刷新尸王殿门(地图, 怪物.当前坐标, 尸王殿门存续秒数)
            if 门 ~= nil then
