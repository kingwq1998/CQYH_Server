...
-- 尸王殿临时传送门: 刷出 8479(尸王殿入口)怪物作触碰传送门, 禁止复活并设存活秒数; 到期由 怪物实例 的 禁止复活+存活时间 路径自动删除
function 刷新尸王殿门(地图实例, 出生坐标, 存活秒数)
  local 模板 = 游戏怪物.获取游戏怪物("尸王殿入口")
  if 模板 == nil then
    return nil
  end
  local 怪物 = 怪物实例(模板, 地图实例, 60000, 出生坐标, true, true)
  if 怪物 ~= nil and 存活秒数 ~= nil and 存活秒数 > 0 then
    怪物.存活时间 = 主程.当前时间:AddSeconds(存活秒数)
  end
  return 怪物
end
